        ...
        if (response.error) {
            ...
            $.each(response.response.tasks, function (index, value) {
                ...
                html +=
                    ...
                    '<select class="form-select" aria-label="Default select example" id="status_task" onchange="set_status(' + value.id + ',this)">' +
                    '<option value="0"' + status_task[0] + '>To do</option>' +
                    '<option value="1"' + status_task[1] + '>Doing</option>' +
                    '<option value="2"' + status_task[2] + '>Done</option>' +
                    '</select>' +
                    '</div>' +
                    '<button type="button" class="btn btn-danger" onclick="del_task(' + value.id + ')">Delete</button>' +
                    '</div>' +
                    '</div>' +
                    '</div>';
            });
